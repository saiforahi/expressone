...
@foreach ($boxes as $key => $row)
    <div class="row" style="background: #f3f3f3;margin-bottom: 1em; padding: 5px;">
        ...
        <div class="col-md-4">
            <button class="btn btn-xs btn-default form-control"
                onclick="dispatch_shipments(<?php echo $row->id; ?>)">Parcels</button>
            <button class="btn btn-xs btn-info form-control rmtd{{ $row->id }}"
                onclick="changeStatus(<?php echo $row->id; ?>)"><i class="fa fa-long-arrow-left"></i> Move </button>
        </div>
    </div>
@endforeach
@if ($boxes->count() < 1)
...
<script>
    $('#assignBulk').keypress(function(event) {
        ...
        if (keycode == '13') {
            ...
            $.ajax({
                type: "get",
                url: '/admin/return-change-box-status-bulk-id/' + bulk_id + '/on-transit',
                success: function(data) {
                    $(".status-dispatch").load('/admin/return-status-dispatch/<?php echo $hub->id; ?>');
                    $(".status-on-transit").load(
                        '/admin/return-status-on-transit/<?php echo $hub->id; ?>');
    ...
    function changeStatus(box_id) {
        ...
        $.ajax({
            type: "get",
            url: '/admin/return-change-box-status/' + box_id + '/dispatch',
            success: function(data) {
                $(".status-dispatch").load('/admin/return-status-dispatch/<?php echo $hub->id; ?>');
