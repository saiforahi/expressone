@foreach ($shipments as $item)
    <table class="table tbl{{$item->id}}">
        ...
        <tr><td><b>Tracking Code: </b>{{$item->tracking_code}}
        <b class="pull-right">Pickup Unit: {{$item->pickup_location->point->unit->name}}</b>
        </td></tr>
        <tr><td><b>Parcel Value: </b>{{$item->amount}}
            <button class="btn btn-info btn-xs pull-right m{{$item->id}}" 
                onclick="moveback(<?php echo $item->id.','.$item->pickup_location->point->unit->id;?>)">Move <i class="fa fa-long-arrow-right"></i> </button>
        </td></tr>
    </table>
@endforeach
