...
@foreach($boxes as $key=>$box)
    @foreach(explode(',',$box->shipment_ids) as $shipment_id)
        ...
        // echo is_avail_agentSide($box->id,$shipment->id).'<br/>';
        ?>

        @if(is_avail_agentSide($box->id,$shipment->id) ==0)
            ...
            <table class="table table-xs" id="{{$shipment->id}}">
                ...
                <tr>
                    ...
                    <td></td>
                    <td class="text-right">
                        Weight: <input style="width:80px;height:17px" type="number" readonly=""
                                       value="{{$shipment->weight}}"></td>
                </tr>
                <tr>
                    <td>Phone: {{$shipment->phone}} </td>
                    <td></td>
                    <td class="text-right hub0">Hub: {{hub_from_area($shipment->area_id)->name}}</td>
                </tr>
                <tr>
                ...
                <tr class="text-right">
                    <td class="text-left" style="width:50%">Date:
                        <small>{{date('M d, Y',strtotime($shipment->created_at))}} (<b
                                class="text-info">{{$shipment->created_at->diffForHumans()}}</b>)</small></td>
                    <td></td>
                    <td>
                        <a href="{{route('shipment-print',$shipment->id)}}" class="btnPrint">
                            <button class="btn btn-warning btn-xs"><i class="fa fa-print"></i> Print</button>
                        </a>&nbsp;
                        <button class="btn btn-success btn-xs addBtn{{$shipment->id}}"
                                onclick="moveToDriverAssign(<?php echo $box->id . ',' . $shipment->id?>);">Move <i
...
<script type="text/javascript">
    ...
    function moveToDriverAssign(box_id, shipment_id) {
        ...
        $.ajax({
            type: "get", url: '/admin/agentDispatch-to-driverAssign/' + box_id + '/' + shipment_id,
