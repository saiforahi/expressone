    ...
    <div class="right_col" role="main">
        <div class="">
            <div class="page-title">
                <div class="title_left"> <h3>Vision page setup</h3></div>
            ...
            <div class="row">
                <div class="col-12">
                    <div class="x_panel">
                      ...
                      <p class="alert text-left alert-info"> Vision page basic data-form with extra contents
                      ...
                      <form id="data1" action="{{route('save-vision-1')}}" method="post" class="x_content collapse @if(session()->has('message'))in @endif" enctype="multipart/form-data">@csrf

                         <textarea id="description" name="description" class="form-control description">{!! old('description')??$about->description !!}</textarea> <br><br>

                         @if($about->id==null)
                          <button class="btn btn-info pull-right" name="submit" value="save" type="submit">Setup vision page</button>
                          @else <button class="btn btn-info pull-right" name="submit" value="update" type="submit">Update vision page</button> @endif
                        </form>
                    </div>
                </div>
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalLabel">Create new post for <b>vision</b>
      ...
      <div class="modal-body">
        <form id="slider_form" method="post" enctype="multipart/form-data">@csrf
          @include('admin.website_manage.vision.form') <br>
...
<div class="modal fade edit_post" tabindex="-1" role="dialog" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel">Edit Vision post
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
              <span aria-hidden="true">&times;</span>
            </button> </h5>
        </div>
        <div class="modal-body">
            <form id="edit_post" method="post" enctype="multipart/form-data">@csrf
                @include('admin.website_manage.vision.form')<br><br>
  ...
  <script>
        ...
        $('.modal-body').css('opacity','0.4');
          ...
          $.ajax({
            ...
            success:function(data){
              ...
              $("[type='submit']").html('<i class="fa fa-check"></i> Save Vision');
      ...
      let table = $('.table').DataTable(
        {
          ...
          serverSide: true,ajax: "{{route('vision-posts')}}",
          order: [ [0, 'desc'] ],
          columns: [
    ...
    $('#dataTable').on('click', '.delete' ,function(e){
       if(confirm('Are you sure to remove the admin record??')){
        let id = $(this).attr('id')
        $.ajax({
           url:"/admin/delete-vision-post/"+id+"", dataType:"json",
           success:function(data){
